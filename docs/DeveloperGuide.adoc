...
The following sequence diagram show hows the Hashtable is updated after a an `add` command:
...
. Fix `invalidAndValidPersonAddressBook.json`, `typicalPersonsParticipantAddressBook.json`, `validAddressBook.json` etc., such that the JSON tests will not fail due to a missing `remark` field.
...
|`* * *` |user |find a person by name |locate details of participants without having to go through the entire list

|`* * *` |user |create Houses and Groups |allocate students to their groupings

|`* * *` |user |delete Houses or Groups |keep my data compact if the group is not needed anymore
...
|`*` |user |input a color-coded schedule |plan what is happening on each day of the camp
|=======================================================================
...
1. User requests to add house
2. User requests to add group to house
3. User requests to add person and specifies group that exists in house
...
. Deleting a person while all participants are listed
